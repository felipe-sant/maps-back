...
class IbgeAPI {
    ...
    public async getMalhaUFs(intrarregiao: boolean = true): Promise<GeoJson> {
        ...
        if (!response.content) throw new Error("Erro na api do ibge ao pegar malha por país")
    ...
    public async getMalhaPerUF(uf: number): Promise<GeoJson> {
        ...
        if (!response.content) throw new Error("Erro na api do ibge ao pegar malha por uf")
        createFile(cache_url, JSON.stringify(response.content))
        return response.content
    }
    ...
    public async getMalhaMunicipioPerUF(uf: number): Promise<GeoJson> {
        ...
        if (!response.content) throw new Error("Erro na api do ibge ao pegar malha de municipio por uf")
        createFile(cache_url, JSON.stringify(response.content))
        return response.content
    }
    ...
    public async getLocalidadePerEstado(estado: number): Promise<EstadoInfo | undefined> {
        ...
        if (response.status !== 200) throw new Error("Erro na api do ibge de pegar localidade por estado")
        if (!response.content) throw new Error("Erro na api do ibge de pegar localidade por estado")
    ...
    public async getLocalidadePerMunicipio(municipio: number): Promise<MunicipioInfo | undefined> {
        ...
        const file = readFile(cache_url)
        if (file) return JSON.parse(file)
        const query = { view: "nivelado" }
        const response = await ReqFunc.getReq<MunicipioInfo_brute | []>(this.url_localidades_municipios + municipio, query)
        if (response.status !== 200) throw new Error("Erro na api do ibge de pegar localidade por municipio")
        if (!response.content) throw new Error("Erro na api do ibge de pegar localidade por municipio")
        if (Array.isArray(response.content)) return undefined
    ...
    public async getPopulacaoPerCode(code: number, periodo?: Periodo): Promise<PopulacaoInfo[]> {
        ...
        if (!response.content) throw new Error("Erro na api do ibge ao pegar informação de população geral")
        ...
        populacaoBrute.forEach(p => {
            ...
            if (periodo) {
                if (p.D2C === periodo) { infoPopulacao.push(formatInfoPopulacao) }
            } else { infoPopulacao.push(formatInfoPopulacao) }
        })
